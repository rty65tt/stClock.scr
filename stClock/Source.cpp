...
static int d_size, b_step, xh, yh;
static int d_radius, wr;
...
void draw_dot(HDC *hdc, int x, int y, COLORREF *c)
{
	if (fChildPreview) {
		SetPixelV(*hdc, x, y, *c);
	}
		...
		g.FillEllipse(&brush, x, y, d_size, d_size);
		g.DrawEllipse(&pen, x, y, d_size, d_size);
	}
}

void draw_symbol(HDC *hdc, RECT &l_rc, unsigned int m, int *x, int *y, COLORREF *d_color, char *p_matrix, int cel = 5, int row = 9)
{
	//int a = m;
	//wchar_t buffer[256];
	...
	rect.right = b_step * cel;
	rect.bottom = b_step * row;
	...
	while (1)
	{
		...
		do {
			if (p_bm[caunter] != 48)
			{
				draw_dot(&hDCMem, bm_x, bm_y, d_color);
			}
			bm_x += b_step;
			++caunter;
		} while (caunter < cel);
...
void draw_second_circle(HDC *hdc, int m)
{
	...
	do {
		int a = count + 45;
		double r = a * 6 * PI / 180;
		xdr = xh + (wr * cos(r)) - d_radius;
		ydr = yh + (wr * sin(r)) - d_radius;
		...
		if (count % 5 == 0)
		{
			xdr = xh + ((wr - b_step) * cos(r)) - d_radius;
			ydr = yh + ((wr - b_step) * sin(r)) - d_radius;
			draw_dot(hdc, (int)xdr, (int)ydr, &dcolor);

			xdr = xh + ((wr - b_step * 2) * cos(r)) - d_radius;
			ydr = yh + ((wr - b_step * 2) * sin(r)) - d_radius;
			draw_dot(hdc, (int)xdr, (int)ydr, &dcolor);
		}
		++count;
	} while (count <= m);
...
LRESULT WINAPI ScreenSaverProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		d_size = (digit_second) ? w_w / 80 : w_w / 60;

		d_radius = d_size / 2;

		b_step = d_size + d_size / 5;

		d_ch_width = b_step * 5;
		d_ch_height = b_step * 9;

		y_pos = yh - d_ch_height / 2;
		if (show_date)
		{
			y_pos = yh - d_ch_height / 2 - b_step * 3;
			ywd_pos = b_step * 11 + y_pos;
		}
